    ...
    public abstract class TTUIPage
    {
        ...
        /// <summary>
        /// record this ui load mode.async(异步) or sync（同步）.
        /// </summary>
        private bool isAsyncUI = false; 

        /// <summary>
        /// this page active flag（标记）
        /// </summary>
        ...
        protected void Show()
        {
            ...
            if (this.gameObject == null && string.IsNullOrEmpty(uiPath) == false)
            {
                ...
                //虚方法，子类可以重写，为该UI的子物体（buttons）赋值，（添加委托）
            ...
            //虚方法，子类没有重写，空方法（时将访问父类方法）  使用时自行添加
            Refresh();
        ...
        private static void PopNode(TTUIPage page)
        {
            ...
            for (int i = 0; i < m_currentPageNodes.Count; i++)
            {
                if (m_currentPageNodes[i].Equals(page))
                {
                    ...
                    //Debug.Log("<color=yellow>"+m_currentPageNodes[0].name+"</color>");
                    _isFound = true;
                    break;
                }
            }
